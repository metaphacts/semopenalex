...
print(f"Overall works entity start -- {start_time}.")

today = date.today()
start_time = time.ctime()
print('works entity started to transform at: '+ start_time)
            ...
            for line in f:
                try:
                    ...
                    if not work_publication_year is None:
                        works_graph.add((work_uri,publication_year_predicate,Literal(work_publication_year,datatype=XSD.integer)))
                    ...
                    if not work_pmcid_id is None:
                        works_graph.add((work_uri,pubmed_central_predicate,Literal(work_pmcid_id,datatype=XSD.string)))
                    ...
                    if not work_host_venue is None:
                        ...
                        if not work_host_venue_id is None:
                            ...
                            work_host_venue_uri = URIRef(soa_namespace_host_venues+str(work_id)+str(work_host_venue_id))
                            ...
                            if not work_host_venue_url is None:
                                ...
                                works_graph.add((work_host_venue_uri,url_predicate,Literal(work_host_venue_url,datatype=XSD.string)))

                            work_host_venue_is_oa = json_data['host_venue']['is_oa']
                            if not work_host_venue_is_oa is None:
                                works_graph.add((work_host_venue_uri,is_oa_predicate,Literal(work_host_venue_is_oa,datatype=XSD.boolean)))
                            ...
                            if not work_host_venue_version is None:
                                ...
                                works_graph.add((work_host_venue_uri,version_predicate,Literal(work_host_venue_version,datatype=XSD.string)))
                            ...
                            if not work_host_venue_license is None:
                                ...
                                works_graph.add((work_host_venue_uri,DCTERMS.license,Literal(work_host_venue_license,datatype=XSD.string)))

                    # type
                    work_type = json_data['type']
                    if not work_type is None:
                        work_type = str("https://api.crossref.org/types/"+str(work_type))
                        works_graph.add((work_uri,crossref_type_predicate,Literal(work_type,datatype=XSD.string)))
                    ...
                    if not work_open_access is None:
                        ...
                        if not work_is_oa is None:
                            works_graph.add((work_open_access_uri,is_oa_predicate,Literal(work_is_oa,datatype=XSD.boolean)))

                        work_oa_status = json_data['open_access']['oa_status']
                        if not work_oa_status is None:
                            works_graph.add((work_open_access_uri,oa_status_predicate,Literal(work_oa_status,datatype=XSD.string)))

                        work_oa_url = json_data['open_access']['oa_url']
                        if not work_oa_url is None:
                            work_oa_url = clean_url(work_oa_url)
                            works_graph.add((work_open_access_uri,oa_url_predicate,Literal(work_oa_url,datatype=XSD.string)))
                    ...
                    if not work_authorships is None:
                        for authorship in work_authorships:
                            ...
                            authorship_id = authorship["author"]["id"].replace("https://openalex.org/", "")
                            work_author_position_uri = URIRef(soa_namespace_author_position+str(work_id)+str(authorship_id))
                            ...
                            works_graph.add((work_author_position_uri,author_position_predicate,Literal(author_position,datatype=XSD.string)))
                    ...
                    if not work_cited_by_count is None:
                        works_graph.add((work_uri,cited_by_count_predicate,Literal(work_cited_by_count,datatype=XSD.integer)))
                    ...
                    if not work_biblio_volume is None:
                        ...
                        works_graph.add((work_uri,has_volume_predicate,Literal(work_biblio_volume,datatype=XSD.string)))

                    work_biblio_issue = json_data['biblio']['issue']
                    if not work_biblio_issue is None:
                        work_biblio_issue = clean(work_biblio_issue)
                        works_graph.add((work_uri,has_issue_predicate,Literal(work_biblio_issue,datatype=XSD.string)))
                    ...
                    if not work_biblio_first_page is None:
                        ...
                        works_graph.add((work_uri,starting_page_predicate,Literal(work_biblio_first_page,datatype=XSD.string)))
                    ...
                    if not work_biblio_last_page is None:
                        ...
                        works_graph.add((work_uri,ending_page_predicate,Literal(work_biblio_last_page,datatype=XSD.string)))

                    # is_retracted
                    work_is_retracted = json_data['is_retracted']
                    if not work_is_retracted is None:
                        works_graph.add((work_uri,is_retracted_predicate,Literal(work_is_retracted,datatype=XSD.boolean)))

                    # is_paratext
                    work_is_paratext = json_data['is_paratext']
                    if not work_is_paratext is None:
                        works_graph.add((work_uri,is_paratext_predicate,Literal(work_is_paratext,datatype=XSD.boolean)))
                    ...
                    if not work_concepts is None:
                        for concept in work_concepts:
                            ...
                            works_graph.add((concept_score_uri,score_predicate,Literal(concept_score,datatype=XSD.integer)))
                    ...
                    if not work_alternate_host_venues is None:
                        for alt_venue in work_alternate_host_venues:
                            if not alt_venue["id"] is None:
                                ...
                                work_alt_host_venue_uri = URIRef(soa_namespace_host_venues+str(work_id)+str(work_alt_venue_id))
                                ...
                                if not alt_venue["url"] is None:
                                    ...
                                    works_graph.add((work_alt_host_venue_uri,url_predicate,Literal(alt_venue_url,datatype=XSD.string)))

                                if not alt_venue["is_oa"] is None:
                                    alt_venue_is_oa = alt_venue["is_oa"]
                                    works_graph.add((work_alt_host_venue_uri,is_oa_predicate,Literal(alt_venue_is_oa,datatype=XSD.boolean)))

                                if not alt_venue["version"] is None:
                                    alt_venue_version = alt_venue["version"]
                                    alt_venue_version = clean(alt_venue_version)
                                    works_graph.add((work_alt_host_venue_uri,version_predicate,Literal(alt_venue_version,datatype=XSD.string)))

                                if not alt_venue["license"] is None:
                                    alt_venue_license = alt_venue["license"]
                                    alt_venue_license = clean(alt_venue_license)
                                    works_graph.add((work_alt_host_venue_uri,DCTERMS.license,Literal(alt_venue_license,datatype=XSD.string)))
                    ...
                    if not abstract_inverted_index is None:
                        ...
                        works_graph.add((work_uri,DCTERMS.abstract,Literal(abstract_inverted_index,datatype=XSD.string)))
